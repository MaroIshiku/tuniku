        ...
        <section className="port-list">
          ...
          {props.ports.map((port) => (
            <article className="port-row" key={port.id}>
              <div className="port-primary"><span className="port-icon"><Icon name="ports" /></span><div><strong>{port.label}</strong><span>{t("localOnly")}{port.notes ? ` · ${port.notes}` : ""}</span></div></div>
